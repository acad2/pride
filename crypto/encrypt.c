...
unsigned char modular_subtraction(unsigned char x, unsigned char y)
{
    return (0);
}
...
int shuffle_bytes(unsigned char* state)   
{
    ...
    for (index = 0; index < 16; index++)
    {
        state[index] = temp[index];
    }    
}
    
int p_box_half(unsigned char* state)
{
...
int test_pbox()
{
    ...
    for (index = 0; index < 16; index++)
    {
        if (state[index] != state_copy[index])
            {
            printf("Failure\n");            
            }
    }
