...
def variable_length_hash(state, rate, output_size, mixing_subroutine, absorb_mode, tweak, _state):    
    output = state[:rate]
    while len(output) < output_size:
        _state = mixing_subroutine(tweak, state, _state)        
        output += state[:rate]      
    return bytes(output[:output_size])
    
def prng_generator(state, rate, output_size, mixing_subroutine, absorb_mode, tweak, _state):    
    while True:
        yield state[:rate]
        _state = mixing_subroutine(tweak, state, _state)    
    
def encryption_generator(state, rate, output_size, mixing_subroutine, absorb_mode, tweak, _state):    
...
def rng_absorb(data, state, rate, mixing_subroutine, absorb_subroutine, tweak, _state): 
...
def rng_sponge_function(hash_input, tweak=tuple(range(256)), key='', output_size=32, capacity=32, rate=224, 
                        mixing_subroutine=rng.shuffle_extract,
    ...
    if key:
        _state = rng_absorb(key, state, rate, mixing_subroutine, absorb_mode, tweak, _state)      
    
    hash_input = pad_input(hash_input, rate)
        
    for _bytes in slide(hash_input, rate):
        _state = rng_absorb(_bytes, state, rate, mixing_subroutine, absorb_mode, tweak, _state)
    
    _state = mixing_subroutine(tweak, state, _state)
    return mode_of_operation(state, rate, output_size, mixing_subroutine, absorb_mode, tweak, _state)
